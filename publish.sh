...
# Inject Clerk publishable key into the auth adapter.
# Set CLERK_PUBLISHABLE_KEY in your Vercel (or local) environment.
if [[ -n "${CLERK_PUBLISHABLE_KEY:-}" ]]; then
  sed -i "s|__CLERK_PUBLISHABLE_KEY__|${CLERK_PUBLISHABLE_KEY}|g" "$DIST/vendor/tinyworld-auth.js"
  printf '✓ Clerk publishable key injected\n'
else
  printf 'Warning: CLERK_PUBLISHABLE_KEY not set — auth will not work in this build\n' >&2
fi
